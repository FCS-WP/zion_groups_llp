...
class AdvancedTitleWidget extends BaseWidget {
	...
	protected function register_content_controls_general() {
		...
		$repeater->add_control(
			'gallery_interval',
			[
				'label' => __('Gallery Interval', 'trx_addons'),
				'type' => Controls_Manager::SLIDER,
				'default' => [
					'size' => 4000,
				],
			...
			switch ( $item['type'] ) {
				...
				case 'video':
					if ( ! empty( $item['video']['url'] ) ) {
						...
						echo '<video class="char" src="' . esc_url( $item['video']['url'] ) . '" nocontrols muted="muted" loop="loop" playsinline="playsinline" autoplay="autoplay" onloadeddata="this.muted=true; this.play();"></video>';
						echo $end_tag;
					}
					break;
				default:
		...
		settings.content.forEach( function( item ) {
			...
			switch ( item.type ) {
				...
				case 'gallery':
					...
					var image_url = '';
					#>{{{ separator }}}{{{ start_tag }}} class="trx-addons-advanced-title-item trx-addons-advanced-title-item-image elementor-repeater-item-{{{ item._id }}}"
				...
				case 'video':
					#>{{{ separator }}}{{{ start_tag }}} class="trx-addons-advanced-title-item trx-addons-advanced-title-item-video elementor-repeater-item-{{{ item._id }}}"><#
						#><video class="char" src="{{{ item.video.url }}}" muted="muted" loop="loop" playsinline="playsinline" autoplay="autoplay" onloadeddata="this.muted=true; this.play();"></video><#
					#>{{{ end_tag }}}<#
					break;
				default:
					break;
