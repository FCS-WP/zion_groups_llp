...
if ( ! function_exists( 'trx_addons_elm_add_params_expand_collapse' ) ) {
	...
	function trx_addons_elm_add_params_expand_collapse( $element, $section_id, $args ) {
		...
		if ( in_array( $element->get_name(), array( 'container', 'section', 'column', 'common', 'common-optimized' ) ) && $section_id == '_section_responsive' ) {
			...
			$element->add_responsive_control( 'trx_expcol_button_padding', array(
				...
				'selectors' => array(
					'{{WRAPPER}} .trx_expcol_button' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}}',
				),
			) );
...
//					),
			...
			$element->add_group_control( \Elementor\Group_Control_Box_Shadow::get_type(), array(
				'name' => 'trx_expcol_collapsed_shadow',
					'label' => esc_html__( 'Shadow', 'elementor' ),
				'selector' => '{{WRAPPER}}.trx_expcol_state_collapsed .trx_expcol_button',
				'condition' => array(
					'trx_expcol_allow' => 'on',
				),
			) );
...
//					),
			...
			$element->add_group_control( \Elementor\Group_Control_Box_Shadow::get_type(), array(
				'name' => 'trx_expcol_expanded_shadow',
					'label' => esc_html__( 'Shadow', 'elementor' ),
				'selector' => '{{WRAPPER}}.trx_expcol_state_expanded .trx_expcol_button',
				'condition' => array(
					'trx_expcol_allow' => 'on',
				),
			) );
...
if ( !function_exists( 'trx_addons_elm_add_expand_collapse_data' ) ) {
	...
	add_action( 'elementor/frontend/container/before_render', 'trx_addons_elm_add_expand_collapse_data', 10, 1 );
