	...
	function trx_addons_load_options() {
		...
		if ( isset( $TRX_ADDONS_STORAGE['options'] ) && is_array( $TRX_ADDONS_STORAGE['options'] ) ) {
			foreach ( $TRX_ADDONS_STORAGE['options'] as $k => $v ) {
				if ( isset( $v['std'] ) ) {
					$val = isset( $_GET[$k] ) && ! is_array( $v['std'] ) && ! apply_filters( 'trx_addons_is_search_page', is_search() )
								? $_GET[$k] 
								: ( isset( $options[$k] )
									? $options[$k]
									: $v['std']
...
if ( ! function_exists( 'trx_addons_get_option' ) ) {
	 ...
	 * @param bool   $from_get  If true - override option value from $_GET array
	 * 
	 * @return mixed         Option value
	 */
	function trx_addons_get_option( $name, $defa = '', $strict_mode = true, $from_get = true ) {
		global $TRX_ADDONS_STORAGE;
		static $options = false;
		$rez = $defa;
		$name = apply_filters( 'trx_addons_filter_get_option_name', $name );
		$part = '';
		...
		if ( $from_get && isset( $_GET[ $name ] ) && ! apply_filters( 'trx_addons_is_search_page', false ) ) {
			if ( empty( $part ) ) {
				$rez = $_GET[ $name ];
...
if ( ! function_exists( 'trx_addons_init_options' ) ) {
	...
	function trx_addons_init_options() {
		...
		$TRX_ADDONS_STORAGE['options'] = apply_filters( 'trx_addons_filter_options', array_merge(
			...
			array(
				'images_info' => array(
					"title" => esc_html__('Images', 'trx_addons'),
					"desc" => wp_kses_data( __("Displaying and sizing images", 'trx_addons') ),
					"type" => "info"
				),
				'retina_ready' => array(
				...
				'links_info' => array(
					"title" => esc_html__('Links', 'trx_addons'),
					"desc" => wp_kses_data( __("Animation of internal links and opening external links in a new tab", 'trx_addons') ),
					"type" => "info"
				),
			...
			array(
				...
				'page_preloader_color' => array(
					"title" => esc_html__('Page preloader color',  'trx_addons'),
					"desc" => wp_kses_data( __('Select color for the page preloader. If empty - the default color is used',  'trx_addons') ),
					"dependency" => array(
...
*/
				...
				'menu_cache' => array(
					...
					"desc" => wp_kses_data( __('Use menu caching (reduces page loading time and the number of database requests). Attention! Please, save the menu again after changing permalink settings! Uncheck this option if you use WPML!', 'trx_addons') ),
					"std" => "0",
					"type" => "switch"
				),
			...
			array(
				...
				'api_google_load' => array(
					...
					"desc" => wp_kses_data( __("Uncheck this field to disable loading Google Maps API script if it is loaded by another plugin", 'trx_addons') ),
					"std" => "1",
					"type" => "switch"
				),
				'api_google' => array(
				...
				'api_google_use_advanced_markers' => array(
					"title" => esc_html__('Use a new API for AdvancedMarkers', 'trx_addons'),
					"desc" => wp_kses_data( __("As of February 21, 2024 (version 3.56) google.maps.Marker is deprecated. Google recommends upgrading to the new google.maps.marker.AdvancedMarkerElement class. Google does not yet plan to discontinue support for google.maps.Marker.", 'trx_addons') )
							. '<br>'
							. wp_kses_data( __("Attention! Animation and clustering are not yet supported for new markers.", 'trx_addons') ),
					"std" => "0",
					"type" => "switch"
				),
				'api_google_marker' => array(
					"title" => esc_html__('Marker icon', 'trx_addons'),
					"desc" => wp_kses_data( __('Default icon for a map marker on Google maps', 'trx_addons') ),
					"std" => '',
					"type" => "image"
				),
				'api_google_cluster' => array(
					"title" => esc_html__('Cluster icon', 'trx_addons'),
					"desc" => wp_kses_data( __('Icon for a cluster that combines several nearby markers on Google maps', 'trx_addons') ),
					"std" => '',
					"type" => "image"
				),
			),
			...
			array(
				...
				'api_openstreet_load' => array(
					...
					"desc" => wp_kses_data( __("Uncheck this field to disable loading OpenStreetMap API script and style if it is loaded by another plugin", 'trx_addons') ),
					"std" => "1",
					"type" => "switch"
				),
				'api_openstreet_tiler' => array(
					"title" => esc_html__('OpenStreetMap tiler', 'trx_addons'),
					"desc" => wp_kses_data( __("Select the type of OpenStreetMap tiler", 'trx_addons') ),
					"options" => trx_addons_get_list_sc_osmap_tilers(),
					"std" => "vector",
					"type" => 'radio'
				),
				'api_openstreet_tiler_vector' => array(
					"title" => esc_html__("List of styles for the map tiler", 'trx_addons'),
					"desc" => wp_kses_data( __("Specify the title, slug and URL for a JSON with a map style from any compatible tiler service. Token and maxZoom are optional (Token is required to access some services, MaxZoom is necessary for some tiles)", 'trx_addons') ),
					"dependency" => array(
				...
				'api_openstreet_marker' => array(
					"title" => esc_html__('Marker icon', 'trx_addons'),
					"desc" => wp_kses_data( __('Default icon for a map marker on OpenStreet maps', 'trx_addons') ),
					"std" => '',
					"type" => "image"
				),
				'api_openstreet_cluster' => array(
					"title" => esc_html__('Cluster icon', 'trx_addons'),
					"desc" => wp_kses_data( __('Icon for a cluster that combines several nearby markers on OpenStreet maps', 'trx_addons') ),
					"std" => '',
					"type" => "image"
				)
			),
			...
			array(
				...
				'login_info' => array(
					...
					"desc" => wp_kses_data( __("Specify parameters for User Login and Registration", 'trx_addons') ),
					"type" => "info"
				),
				'login_via_ajax' => array(
					"title" => esc_html__('Login via AJAX', 'trx_addons'),
					"desc" => wp_kses_data( __('Login via AJAX or use a direct link on the WP Login page. Uncheck this if you are experiencing problems with any login plugins.', 'trx_addons') ),
					"std" => "1",
					"type" => "switch"
				),
				'login_via_socials' => array(
					"title" => esc_html__('Login via social profiles',  'trx_addons'),
					"desc" => wp_kses_data( __('Provide a shortcode from your Social Login Plugin or any HTML/JS code to create a Social Login section.',  'trx_addons') ),
					"std" => "",
					"type" => "textarea"
				),
				"notify_about_new_registration" => array(
					"title" => esc_html__('Notify about new registration', 'trx_addons'),
					"desc" => wp_kses_data( __("Send an e-mail with new registration data to the site admin's e-mail address and/or to the new user's e-mail address.", 'trx_addons') ),
					"std" => "no",
					"options" => array(
				...
				"double_opt_in_registration" => array(
					...
					"desc" => wp_kses_data( __("Send an e-mail with a confirmation link to the new user's e-mail address.", 'trx_addons') ),
					"std" => "1",
					"type" => "switch"
				),
			...
			array(
				...
				'share' => array(
					...
					"desc" => wp_kses(
									__("Clone the field group, then select an icon/image, specify the social network's title and URL to share posts.", 'trx_addons')
				...
				'add_og_tags' => array(
					...
					"desc" => wp_kses_data( __("Open Graph tags determine how URLs are displayed (e.g. image, title, description) when shared on social media. They are used by many popular social media sites, such as Facebook.", 'trx_addons') ),
					"std" => "1",
					"type" => "switch"
				),
			),
			...
			! apply_filters( 'trx_addons_filter_allow_ajax_posts_selector', true ) ? array() :
			array(
				'sc_ids_info' => array(
			...
			! apply_filters( 'trx_addons_filter_allow_layouts_in_tabs', true ) ? array() :		// ! trx_addons_elm_is_experiment_active( 'container' ) || ! trx_addons_elm_is_experiment_active( 'nested-elements' )
			array(
				'sc_tabs_info' => array(
				...
				'sc_tabs_layouts' => array(
					...
					"desc" => wp_kses_data( __('Allow to use custom layouts and saved templates inside tabs (only for Elementor\'s old Tabs widget )', 'trx_addons') ),
					"std" => "0",
					"type" => "switch"
				),
			),
			...
			array(
				...
				'page_wrap_class' => array(
					...
					"desc" => wp_kses_data( __("Specify theme-specific class for the entire page wrapper (contains all of the page components).", 'trx_addons') ),
					"std" => '.page_wrap',
					"type" => "text"
				),
			)
