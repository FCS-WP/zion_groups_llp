...
if ( ! function_exists( 'trx_addons_set_images_threshold' ) ) {
	...
	function trx_addons_set_images_threshold( $threshold = 2560 ) {
		$t = (int) trx_addons_get_option( 'images_threshold' );
		...
		$is_external = apply_filters(
							...
							in_array( $ts,
										apply_filters(
											...
											array( 'full', 'post-thumbnail', 'thumbnail', 'medium', 'medium_large', 'large' )   // Don't add 'medium' to this array
										)
									)
...
if ( ! function_exists( 'trx_addons_remove_lazy_load_from_page_content' ) ) {
	add_action( 'after_setup_theme', 'trx_addons_remove_lazy_load_from_page_content', 10 );
	function trx_addons_remove_lazy_load_from_page_content() {
		// Don't process the html output in the admin area and Gutenberg editor (processing is broke the json content inside the page)
		if ( is_admin() || trx_addons_is_preview( 'gutenberg' ) ) {
			return;
		}
		$wrappers = apply_filters( 'trx_addons_filter_remove_loading_lazy_wrappers', array(
			'masonry_item',
			'masonry-item',
		) );
		if ( count( $wrappers ) == 0 ) return;
		ob_start( function( $html ) use ( $wrappers ) {
			if ( ! empty( $html ) && is_string( $html ) ) {
				...
				foreach ( $wrappers as $wrapper ) {
					if ( strpos( $html, $wrapper ) !== false ) {
						// Regular expressions are not working correctly, because the tag <img> can be inside the <div> with the data-image-effect attribute
						// and in some cases between them can be other tags like <div> or <style>
						// $html = preg_replace( '/(<div[^>]*data-image-effect[^>]*>[\s]*(<(div|style)[^>]*>[\s]*){0,3}<img[^>]*)(loading="lazy")/iU', '${1}loading="eager"', $html );
						// $html = preg_replace( '/(<div[^>]*data-image-effect[^>]*>[\s]*(<(div|style)[^>]*>[\s]*){0,3}<img[^>]*)(decoding="async")/iU', '${1}decoding="sync"', $html );
						// So, we need to use a simple string search and replace
						$pos = 0;
						do {
							$pos = strpos( $html, $wrapper, $pos + 20 );
							if ( $pos !== false ) {
								// Find a tag starting position
								$pos_tag_start = 0;
								for ( $i = $pos; $i > 0; $i-- ) {
									if ( $html[$i] == '<' ) {
										$pos_tag_start = $i;
								...
								// Find an image tag starting position
								$pos_img_start = strpos( $html, '<img', $pos_tag_start );
								if ( $pos_img_start !== false ) {
									$pos_img_end = strpos( $html, '>', $pos_img_start + 4 );
									$img_tag_orig = $img_tag = substr( $html, $pos_img_start, $pos_img_end - $pos_img_start + 1 );
									// Replace atts in the original image tag
									foreach ( $atts as $att => $new_att ) {
										$img_tag = str_replace( $att, $new_att, $img_tag );
									}
									if ( $img_tag != $img_tag_orig ) {
										// Add 'fetchpriority="high"' to the image tag
										if ( strpos( $img_tag, 'fetchpriority=' ) === false ) {
											$img_tag = str_replace( '<img ', '<img fetchpriority="high" ', $img_tag );
											$pos += 20;
										}
										// Replace the image tag in the original content
										$html = substr_replace( $html, $img_tag, $pos_img_start, $pos_img_end - $pos_img_start + 1 );
									}
								}
							}
						} while ( $pos !== false );
...
if ( ! function_exists( 'trx_addons_get_attachment_img' ) ) {
	...
	function trx_addons_get_attachment_img( $image_id, $size = 'full', $args = array() ) {
		...
		if ( is_numeric( $image_id ) && (int) $image_id > 0 ) {
			$opts = array( 'class' => $args['class'] );
			if ( isset( $args['loading'] ) ) {
				$opts['loading'] = $args['loading'];
			}
			if ( isset( $args['decoding'] ) ) {
				$opts['decoding'] = $args['decoding'];
			}
			if ( isset( $args['fetchpriority'] ) ) {
				$opts['fetchpriority'] = $args['fetchpriority'];
			}
			$image = wp_get_attachment_image( $image_id, apply_filters('trx_addons_filter_thumb_size', $size, $args['filter'] ), false, $opts );
		} else {
			...
			if ( ! empty( $image ) ) {
				...
				$image = '<img src="' . esc_url( $image ) . '"'
							...
							. ( isset( $args['loading'] ) ? ' loading="' . trim( $args['loading'] ? $args['loading'] : 'eager' ) : '' )
							. ( isset( $args['decoding'] ) ? ' decoding="' . trim( $args['decoding'] ? $args['decoding'] : 'auto' ) : '' )
							. ( isset( $args['fetchpriority'] ) ? ' fetchpriority="' . trim( $args['fetchpriority'] ? $args['fetchpriority'] : 'auto' ) : '' )
		...
		if ( $use_wp_embed && is_object( $wp_embed ) ) {
			...
			if ( strpos( $video, 'player.vimeo.com' ) === false ) {
				...
				*/
				// New way
				$video = preg_replace(
							'~vimeo\.com/(\d+)(?:/([a-z0-9]+))?~i',
							'player.vimeo.com/video/$1?h=$2',
							$video
						);
			}
...
if ( ! function_exists('trx_addons_clear_icon_name' ) ) {
	...
	function trx_addons_clear_icon_name( $icon ) {
		return trx_addons_esc( sanitize_file_name( (string)apply_filters( 'trx_addons_filter_clear_icon_name', str_replace( array( 'trx_addons_icon-', 'icon-' ), '', $icon ) ) ) );
	}
}
