...
if ( ! function_exists('trx_addons_minify_css' ) ) {
	...
	function trx_addons_minify_css( $css, $minify = true ) {
		if ( $minify ) {
			...
			// Attention!
			// 1) Don't remove spaces around '+', because it broke rules with calc( a + b )
			// 2) Don't remove spaces around ':', because it broke rules with pseudo-class :where()
...
if ( ! function_exists( 'trx_addons_loading_layout' ) ) {
	/**
	 * Return a "loading" layout (spinner while posts loading)
	 * 
	 * @param array|boolean  $args  Optional. Arguments to generate the layout.
	 * 						 If it is boolean - it is echo parameter.
	 * 					     If it is an array - available keys:
	 *                       - style: Loading style (default: 'default')
	 *                       - class: Additional classes for the layout (default: '')
	 * 						 - hidden: initial state of the layout (default: false)
	 *                       - echo: If true - echo the layout, else - return it (default: true)
	 * 
	 * @return string  Loading layout
	 */
	function trx_addons_loading_layout( $args = array() ) {
		if ( ! is_array( $args ) ) {
		...
		$args['class'] = 'trx_addons_loading'
						. ' trx_addons_loading_style_' . esc_attr( $args['style'] )
						. ( ! empty( $args['hidden'] ) ? ' trx_addons_hidden' : '' )
						. ( $args['class'] ? ' ' . $args['class'] : '' );
		$layout = apply_filters( 'trx_addons_filter_loading_layout', '<div class="' . esc_attr( $args['class'] ) . '"></div>' );
		if ( $args['echo'] ) {
