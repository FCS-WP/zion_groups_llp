...
if ( function_exists( 'deliciosa_elementor_is_preview' ) && deliciosa_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			deliciosa_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	deliciosa_new_main_query(
		array(
			'post_type'      => deliciosa_get_theme_option( 'post_type' ),
			'category'       => deliciosa_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'deliciosa_filter_get_template_part', deliciosa_blog_archive_get_template() ) );
}
