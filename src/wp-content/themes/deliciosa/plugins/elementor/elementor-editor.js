...
jQuery( document ).ready(
	function() {
		...
		deliciosa_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="deliciosa_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.deliciosa_post_editor' );
						if ( link.length > 0 ) {
							deliciosa_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.deliciosa_post_editor', function(e) {
						deliciosa_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('deliciosa_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function deliciosa_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
