...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			deliciosa_create_observer( 'deliciosa-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				deliciosa_gutenberg_editor_init();
			} );
		} else {
		 	deliciosa_gutenberg_editor_init();
		}
	...
	function deliciosa_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length && editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'deliciosa_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'deliciosa_inited' ) ) {
	...
	// Init on page load
	function deliciosa_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = deliciosa_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + DELICIOSA_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + DELICIOSA_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in DELICIOSA_STORAGE['override_classes'] ) {
				$obj.addClass( DELICIOSA_STORAGE['override_classes'][i].replace( '%s', DELICIOSA_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( deliciosa_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && DELICIOSA_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'deliciosa_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			deliciosa_remove_observer( 'deliciosa-check-editor-styles-wrapper' );
			deliciosa_create_observer( 'deliciosa-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'deliciosa_inited' );
	}
} );
