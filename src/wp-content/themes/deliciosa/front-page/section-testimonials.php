<div class="front_page_section front_page_section_testimonials<?php
	$deliciosa_scheme = deliciosa_get_theme_option( 'front_page_testimonials_scheme' );
	if ( ! empty( $deliciosa_scheme ) && ! deliciosa_is_inherit( $deliciosa_scheme ) ) {
		echo ' scheme_' . esc_attr( $deliciosa_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( deliciosa_get_theme_option( 'front_page_testimonials_paddings' ) );
	if ( deliciosa_get_theme_option( 'front_page_testimonials_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$deliciosa_css      = '';
		$deliciosa_bg_image = deliciosa_get_theme_option( 'front_page_testimonials_bg_image' );
...
<?php
	// Add anchor
	$deliciosa_anchor_icon = deliciosa_get_theme_option( 'front_page_testimonials_anchor_icon' );
	$deliciosa_anchor_text = deliciosa_get_theme_option( 'front_page_testimonials_anchor_text' );
...
?>
	...
	<?php
	if ( deliciosa_get_theme_option( 'front_page_testimonials_fullheight' ) ) {
	...
	?>
			...
			<?php
			$deliciosa_css      = '';
			$deliciosa_bg_mask  = deliciosa_get_theme_option( 'front_page_testimonials_bg_mask' );
			$deliciosa_bg_color_type = deliciosa_get_theme_option( 'front_page_testimonials_bg_color_type' );
			if ( 'custom' == $deliciosa_bg_color_type ) {
				$deliciosa_bg_color = deliciosa_get_theme_option( 'front_page_testimonials_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_testimonials_content_wrap content_wrap">
			<?php
			// Caption
			$deliciosa_caption = deliciosa_get_theme_option( 'front_page_testimonials_caption' );
			if ( ! empty( $deliciosa_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_testimonials_caption front_page_block_<?php echo ! empty( $deliciosa_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $deliciosa_caption, 'deliciosa_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$deliciosa_description = deliciosa_get_theme_option( 'front_page_testimonials_description' );
			if ( ! empty( $deliciosa_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_testimonials_description front_page_block_<?php echo ! empty( $deliciosa_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $deliciosa_description ), 'deliciosa_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_testimonials_output">
				...
				if ( is_active_sidebar( 'front_page_testimonials_widgets' ) ) {
					...
					if ( ! deliciosa_exists_trx_addons() ) {
						...
						deliciosa_customizer_need_widgets_message( 'front_page_testimonials_caption', 'ThemeREX Addons - Testimonials' );
					}
				}
				?>
			</div>
