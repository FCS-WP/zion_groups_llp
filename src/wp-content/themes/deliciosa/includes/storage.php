...
if ( ! function_exists( 'deliciosa_storage_get' ) ) {
	/**
	 ...
	 */
	function deliciosa_storage_get( $var_name, $default = '' ) {
		global $DELICIOSA_STORAGE;
		return isset( $DELICIOSA_STORAGE[ $var_name ] ) ? $DELICIOSA_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'deliciosa_storage_set' ) ) {
	/**
	 ...
	 */
	function deliciosa_storage_set( $var_name, $value ) {
		global $DELICIOSA_STORAGE;
		$DELICIOSA_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'deliciosa_storage_empty' ) ) {
	/**
	 ...
	 */
	function deliciosa_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $DELICIOSA_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $DELICIOSA_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $DELICIOSA_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $DELICIOSA_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'deliciosa_storage_isset' ) ) {
	/**
	 ...
	 */
	function deliciosa_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $DELICIOSA_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $DELICIOSA_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $DELICIOSA_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $DELICIOSA_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'deliciosa_storage_unset' ) ) {
	/**
	 ...
	 */
	function deliciosa_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $DELICIOSA_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			unset( $DELICIOSA_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			unset( $DELICIOSA_STORAGE[ $var_name ][ $key ] );
		} else {
			unset( $DELICIOSA_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'deliciosa_storage_inc' ) ) {
	/**
	 ...
	 */
	function deliciosa_storage_inc( $var_name, $value = 1 ) {
		global $DELICIOSA_STORAGE;
		if ( empty( $DELICIOSA_STORAGE[ $var_name ] ) ) {
			$DELICIOSA_STORAGE[ $var_name ] = 0;
		}
		$DELICIOSA_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'deliciosa_storage_concat' ) ) {
	/**
	 ...
	 */
	function deliciosa_storage_concat( $var_name, $value ) {
		global $DELICIOSA_STORAGE;
		if ( empty( $DELICIOSA_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'deliciosa_storage_get_array' ) ) {
	/**
	 ...
	 */
	function deliciosa_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $DELICIOSA_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $DELICIOSA_STORAGE[ $var_name ][ $key ] ) ? $DELICIOSA_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $DELICIOSA_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $DELICIOSA_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'deliciosa_storage_set_array' ) ) {
	/**
	 ...
	 */
	function deliciosa_storage_set_array( $var_name, $key, $value ) {
		global $DELICIOSA_STORAGE;
		if ( ! isset( $DELICIOSA_STORAGE[ $var_name ] ) ) {
			$DELICIOSA_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$DELICIOSA_STORAGE[ $var_name ][] = $value;
		} else {
...
if ( ! function_exists( 'deliciosa_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function deliciosa_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $DELICIOSA_STORAGE;
		if ( ! isset( $DELICIOSA_STORAGE[ $var_name ] ) ) {
			$DELICIOSA_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $DELICIOSA_STORAGE[ $var_name ][ $key ] ) ) {
			$DELICIOSA_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( '' === $key2 ) {
			$DELICIOSA_STORAGE[ $var_name ][ $key ][] = $value;
		} else {
			$DELICIOSA_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
		}
	}
}

if ( ! function_exists( 'deliciosa_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function deliciosa_storage_merge_array( $var_name, $key, $value ) {
		global $DELICIOSA_STORAGE;
		if ( ! isset( $DELICIOSA_STORAGE[ $var_name ] ) ) {
			$DELICIOSA_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$DELICIOSA_STORAGE[ $var_name ] = array_merge( $DELICIOSA_STORAGE[ $var_name ], $value );
		} else {
			$DELICIOSA_STORAGE[ $var_name ][ $key ] = array_merge( $DELICIOSA_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'deliciosa_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function deliciosa_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $DELICIOSA_STORAGE;
		if ( ! isset( $DELICIOSA_STORAGE[ $var_name ] ) ) {
			$DELICIOSA_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			deliciosa_array_insert_after( $DELICIOSA_STORAGE[ $var_name ], $after, $key );
		} else {
			deliciosa_array_insert_after( $DELICIOSA_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'deliciosa_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function deliciosa_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $DELICIOSA_STORAGE;
		if ( ! isset( $DELICIOSA_STORAGE[ $var_name ] ) ) {
			$DELICIOSA_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			deliciosa_array_insert_before( $DELICIOSA_STORAGE[ $var_name ], $before, $key );
		} else {
			deliciosa_array_insert_before( $DELICIOSA_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'deliciosa_storage_push_array' ) ) {
	/**
	 ...
	 */
	function deliciosa_storage_push_array( $var_name, $key, $value ) {
		global $DELICIOSA_STORAGE;
		if ( ! isset( $DELICIOSA_STORAGE[ $var_name ] ) ) {
			$DELICIOSA_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $DELICIOSA_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $DELICIOSA_STORAGE[ $var_name ][ $key ] ) ) {
				$DELICIOSA_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $DELICIOSA_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'deliciosa_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function deliciosa_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $DELICIOSA_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $DELICIOSA_STORAGE[ $var_name ] ) && is_array( $DELICIOSA_STORAGE[ $var_name ] ) && count( $DELICIOSA_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $DELICIOSA_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $DELICIOSA_STORAGE[ $var_name ][ $key ] ) && is_array( $DELICIOSA_STORAGE[ $var_name ][ $key ] ) && count( $DELICIOSA_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $DELICIOSA_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'deliciosa_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function deliciosa_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $DELICIOSA_STORAGE;
		if ( ! isset( $DELICIOSA_STORAGE[ $var_name ] ) ) {
			$DELICIOSA_STORAGE[ $var_name ] = array();
		}
		if ( empty( $DELICIOSA_STORAGE[ $var_name ][ $key ] ) ) {
			$DELICIOSA_STORAGE[ $var_name ][ $key ] = 0;
		}
		$DELICIOSA_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'deliciosa_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function deliciosa_storage_concat_array( $var_name, $key, $value ) {
		global $DELICIOSA_STORAGE;
		if ( ! isset( $DELICIOSA_STORAGE[ $var_name ] ) ) {
			$DELICIOSA_STORAGE[ $var_name ] = array();
		}
		if ( empty( $DELICIOSA_STORAGE[ $var_name ][ $key ] ) ) {
			$DELICIOSA_STORAGE[ $var_name ][ $key ] = '';
		}
		$DELICIOSA_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'deliciosa_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function deliciosa_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $DELICIOSA_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $DELICIOSA_STORAGE[ $var_name ] ) ? $DELICIOSA_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $DELICIOSA_STORAGE[ $var_name ] ) ? $DELICIOSA_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'deliciosa_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function deliciosa_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $DELICIOSA_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $DELICIOSA_STORAGE[ $var_name ]->$prop ) ? $DELICIOSA_STORAGE[ $var_name ]->$prop : $default;
	}
}
