...
if ( ! function_exists( 'deliciosa_customizer_get_css' ) ) {

	function deliciosa_customizer_get_css( $args = array() ) {
		...
		if ( $fonts ) {
			...
			$fonts        = deliciosa_customizer_add_theme_fonts( $fonts );
			$rez          = array();
...
.deliciosa_tabs .deliciosa_tabs_titles li a {
...
/* Custom style for Reservation form */
...
CSS;
			$rez          = apply_filters( 'deliciosa_filter_get_css', $rez, array( 'fonts' => $fonts ) );
			$css['fonts'] = $rez['fonts'];
		}
		...
		if ( $vars ) {
			...
			$vars = deliciosa_customizer_add_theme_vars( $vars );

			$rez         = array();
			$rez['vars'] = '';
			$rez         = apply_filters( 'deliciosa_filter_get_css', $rez, array( 'vars' => $vars ) );
			$css['vars'] = $rez['vars'];
		}
		...
		//--------------------------------------
		if ( false !== $colors ) {
			$schemes = empty( $scheme ) ? array_keys( deliciosa_get_sorted_schemes() ) : array( $scheme );
			if ( count( $schemes ) > 0 ) {
				$rez = array();
				foreach ( $schemes as $s ) {
					// Prepare colors
					if ( empty( $scheme ) ) {
						$colors = deliciosa_get_scheme_colors( $s );
					}
					...
					$rez['colors']  = '';
					$rez            = apply_filters(
						'deliciosa_filter_get_css', $rez, array(
							'colors' => $colors,
							'scheme' => $s,
						)
					);
		...
		return apply_filters( 'deliciosa_filter_prepare_css', $css_str, $remove_spaces );
	}
}
